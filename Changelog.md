...
### Changes
* CodeGen TPGs (Switch and Stack modes) now matches the execution behaviour of `TPGExecutionEngine`, especially regarding cycle handling. Now, a team can't be visited more than once per inference, and edges leading to an already visited team are not evaluated (their program are not executed).

### Bug fix
* Fix penalty score given to edges in cycle handling `TPGSwitchGenerationEngine`, was `DBL_MIN` but should be `-DBL_MAX`.
