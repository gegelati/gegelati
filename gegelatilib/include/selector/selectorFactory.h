...
namespace Selector {
     ...
     * 
     * \param[in] graph shared pointer of the TPGGraph on which the selection is done.
     * \param[in] le LearningEnvironment used by the LearningAgent.
     * \param[in] params LearningParameters used
     */
    std::shared_ptr<Selector> selectorFactory(const std::shared_ptr<TPG::TPGGraph> graph, const Learn::LearningEnvironment& le, const Learn::LearningParameters& params);
