...
Representation::TPG::TPGAgent& Representation::TPG::TPGManager::getTPGAgentFromCst(const Agent& agent)
{
    return dynamic_cast<TPGAgent&>(**this->getAgentFromCst(agent));
}

const std::vector<std::reference_wrapper<const Representation::Agent>> Representation::TPG::TPGManager::getAgents() const
{
    std::vector<std::reference_wrapper<const Representation::Agent>> constAgents;
...
const Representation::Agent& Representation::TPG::TPGManager::createAgent(EvoGraph::Graph& graph)
{
    return this->createAgent(graph.addNewTeam());
}

const Representation::Agent& Representation::TPG::TPGManager::createAgent(std::optional<std::reference_wrapper<const EvoGraph::Vertex>> vertex)
{
    this->agents.insert(std::make_unique<TPGAgent>(vertex, this->getRepresentationID()));
    return **this->agents.rbegin();
}

const Representation::Agent& Representation::TPG::TPGManager::createEmptyAgent()
{
    std::optional<std::reference_wrapper<const EvoGraph::Vertex>> vertex = std::nullopt;
    return this->createAgent(vertex);
}

const Representation::Agent& Representation::TPG::TPGManager::copyAgent(const Agent& agent, EvoGraph::Graph& graph)
{
    const TPGAgent& castedAgent = dynamic_cast<const TPGAgent&>(agent);
    if(&castedAgent == nullptr){
        throw std::runtime_error("Representation::TPG::TPGManager::copyAgent: trying to copy an agent that is not a TPGAgent.");
    }
    ...
    if(agent.getRepresentationID() != this->getRepresentationID()){
        ...
        for(const EvoGraph::Edge& edge: castedAgent.getVertex().getOutgoingEdges()){
            const Representation::Agent& newSubAgent = this->getSubManager(this->programRepresentationID).copyAgent(edge.getProgram(), graph);
...
void Representation::TPG::TPGManager::deleteAgent(const Agent& agent, EvoGraph::Graph& graph)
{
...
void Representation::TPG::TPGManager::emptyAgent(const Agent& agent, EvoGraph::Graph& graph)
{
    // Do not remove action agents from the graph
    if(auto vertex = dynamic_cast<const EvoGraph::Team*>(&this->getTPGAgentFromCst(agent).getVertex())){
...
void Representation::TPG::TPGManager::setVertex(const Agent& agent, const EvoGraph::Vertex& vertex)
{
...
std::unique_ptr<Representation::ExecutionEngine> Representation::TPG::TPGManager::createExecutionEngine(std::vector<std::reference_wrapper<const Data::DataHandler>> dataSources, bool isTraining) const
{
