...
void Representation::TPG::TPGAgent::setVertex(const EvoGraph::Vertex& newVertex)
{
...
bool Representation::TPG::TPGAgent::isValid() const
{
    if(!this->hasVertex()){
        return false;
...
bool Representation::TPG::TPGAgent::hasVertex() const
{
    return this->vertex != std::nullopt;
}

void Representation::TPG::TPGAgent::removeVertex()
{
    this->vertex == std::nullopt;
}


bool Representation::TPG::TPGAgent::isRoot() const
{
    return this->vertex->get().getIncomingEdges().size() == 0;
}

const EvoGraph::Vertex& Representation::TPG::TPGAgent::getVertex() const
{
    if(!this->vertex){
