...
void ExhaustiveArchive::addRecording(const Program::Program* const program, const std::vector<std::reference_wrapper<DataHandlers::DataHandler>>& dHandler, double result)
{
	// Always archive
	// get the combined hash
	size_t hash = getCombinedHash(dHandler);

	// No check for duplicity: every single recording is stored
	...
	// No fancy recordings per program:
	// this archive is not meant to be used for program divercity purpose.

	// No check for max size either
}

void ExhaustiveArchive::insertAllRecordings(Archive& other) const
{
	for (ArchiveRecording recording : this->recordings) {
		other.addRecording(recording.prog, this->dataHandlers.at(recording.dataHash), recording.result);
	}
}
