...
size_t Data::DataType::totalElements() const noexcept {
    return dimensions[0] * (rank >= 2 ? dimensions[1] : 1);
}

size_t Data::DataType::sourceTotalElements() const noexcept {
    return sourceDimensions[0] * (sourceRank >= 2 ? sourceDimensions[1] : 1);
}

bool Data::DataType::canFitIn(const DataType& requested, size_t offset) const noexcept {
...
size_t Data::DataType::getAddressSpace(const DataType& requested) const noexcept
{
    if(!this->canFitIn(requested, 0)) {
        ...
        // Requested is scalar since it fits
        return 1;
    }

    else if(this->rank == 1) {
        // TODO Small opti possible
        if(requested.rank == 0) {
            return this->dimensions[0];
        } else {
            // Rank == 1 since it fits
            return this->dimensions[0] - requested.dimensions[0] + 1;
        }
    }

    // Case this rank = 2
    else if (requested.rank == 0) {
        return this->totalElements();
    } else if (requested.rank == 1) {
        return (this->dimensions[1] - requested.dimensions[0] + 1) * this->dimensions[0];
    } else {
...
bool Data::DataType::equalsWithSource(const DataType& other) const noexcept {
    return *this == other &&
...
std::string Data::DataType::toString() const {
...
std::string Data::DataType::summary() const {
    std::ostringstream oss;
    if (!this->elementType) {
        oss << "'unknown";
    } else {
