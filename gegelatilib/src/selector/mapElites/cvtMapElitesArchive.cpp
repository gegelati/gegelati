...
double Selector::MapElites::CvtMapElitesArchive::dist_squared(const std::vector<double>& a, const std::vector<double>& b) {
    double sum = 0.0;
...
std::vector<double> Selector::MapElites::CvtMapElitesArchive::add(const std::vector<double>& a, const std::vector<double>& b) {
...
std::vector<double> Selector::MapElites::CvtMapElitesArchive::scalar_mult(const std::vector<double>& a, double s) {
    std::vector<double> res(this->nbDescriptors);
    for (size_t i = 0; i < this->nbDescriptors; ++i)
        res[i] = a[i] * s;
    return res;
...
size_t Selector::MapElites::CvtMapElitesArchive::nearest(const std::vector<double>& point, const std::vector<std::vector<double>>& centroids) {
...
void Selector::MapElites::CvtMapElitesArchive::initialize_cvt(Mutator::RNG& rng)
{
    ...
    std::vector<size_t> j(this->nbCentroids, 1); // Compteur d'updates par centroïde

    for (size_t iter = 0; iter < this->nbIterationInit; ++iter) {

        // print progress with a line overrite at each iteration
        std::cout << "\rIteration " << iter + 1 << "/" << this->nbIterationInit << " (centroids initialized: " << j.size() << ")"<< std::flush;
        std::vector<std::vector<double>> samples(this->nbDotsInit);
        std::vector<std::vector<std::vector<double>>> assignments(this->nbCentroids);
        ...
        for (size_t i = 0; i < this->nbCentroids; ++i) {
            if (!assignments[i].empty()) {
                ...
                for (size_t d = 0; d < this->nbDescriptors; ++d) {
                    new_z[d] = ((this->a1 * j_i + this->b1) * z_i[d] + (this->a2 * j_i + this->b2) * u_i[d]) / (j_i + 1);
                }
...
void Selector::MapElites::CvtMapElitesArchive::initCSVarchive(std::string archivePath) const {
...
void Selector::MapElites::CvtMapElitesArchive::updateCSVArchive(std::string path, uint64_t generationNumber) const {
...
Selector::MapElites::CvtMapElitesArchive::getArchiveFromDescriptors(const std::vector<double>& descriptors) const
{
