    ...
    const Learn::LearningParameters& params)
{
    ...
    if (params.selection._selectionMode == "classification") {
        return std::make_shared<ClassificationSelector>(manager, params, 1);
    }
    else if (params.selection._selectionMode == "truncation") {
        return std::make_shared<TruncationSelector>(manager, params);
    }
    else if (params.selection._selectionMode == "tournament") {
        return std::make_shared<TournamentSelector>(manager, params);
    }
        ...
        return std::make_shared<MapElitesSelector>(manager, params);
    }
    else {
