...
TEST(DataHandlersTest, PrimitiveDataArrayCanProvide) {
	...
	ASSERT_FALSE(d->canHandle(typeid(Data::SupportedType))) << "PrimitiveTypeArray<double>() wrongfully say it can provide SupportedType data.";

	ASSERT_TRUE(d->canHandle(typeid(Data::PrimitiveType<double>[2]))) << "PrimitiveTypeArray<double>() wrongfully says it can not provide an array of its PrimitiveType<double>[2].";
	ASSERT_FALSE(d->canHandle(typeid(Data::PrimitiveType<double>[5]))) << "PrimitiveTypeArray<double>() wrongfully says it can provide an array of its PrimitiveType<double>[5].";
...
TEST(DataHandlersTest, PrimitiveDataArrayAddressSpace) {
	...
	ASSERT_EQ(d->getAddressSpace(typeid(Data::PrimitiveType<long>[62])), 3) << "Address space size for type PrimitiveType<long>[62] in PrimitiveTypeArray<long>(64) is not 3";
...
TEST(DataHandlersTest, PrimitiveDataArrayGetDataAt) {
	...
	for (int i = 0; i < size; i++) {
		const Data::PrimitiveType<float> a = *std::dynamic_pointer_cast<const Data::PrimitiveType<float>>(d->getDataAt(typeid(Data::PrimitiveType<float>), i));
...
TEST(DataHandlersTest, PrimitiveDataArraySetDataAt) {
	...
	ASSERT_EQ((double)*(std::dynamic_pointer_cast<const Data::PrimitiveType<double>>(d->getDataAt(typeid(Data::PrimitiveType<double>), address))), doubleValue) << "Previously set data did not persist.";
...
TEST(DataHandlersTest, PrimitiveDataArrayGetDataAtGetArray) {
	...
	d->resetData();
	for (int i = 0; i < size; i++) {
		Data::PrimitiveType<double> value(i);
		d->setDataAt(typeid(value), i, value);
	}

	const size_t arraySize{ 10 };
	for (int i = 0; i < size - arraySize + 1; i++) {
		std::shared_ptr< const Data::PrimitiveType<double>[arraySize]> a = std::dynamic_pointer_cast<const Data::PrimitiveType<double>[arraySize]>(d->getDataAt(typeid(Data::PrimitiveType<double>[arraySize]), i));
	...
	ASSERT_THROW(d->getDataAt(typeid(Data::PrimitiveType<double>[arraySize]), (size - arraySize + 1)), std::out_of_range) << "Address exceeding the addressSpace should cause an exception.";
	ASSERT_THROW(d->getDataAt(typeid(Data::PrimitiveType<int>[2]), 0), std::invalid_argument) << "Requesting a non-handled type, even at a valid location, should cause an exception.";
