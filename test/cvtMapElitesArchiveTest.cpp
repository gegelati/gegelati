...
protected:
    ...
    void SetUp() override {
        archive = new Selector::MapElites::CvtMapElitesArchive(rng, centroids, descriptors, minV, maxV, iter, dots, a1, b1, a2, b2);
        eval1 = std::make_shared<Learn::EvaluationResult>(std::make_shared<Selector::SelectionMetrics>(0), 1);
        eval2 = std::make_shared<Learn::EvaluationResult>(std::make_shared<Selector::SelectionMetrics>(1), 1);
    }
...
TEST_F(CvtMapElitesArchiveTest, DistSquared) {
    ...
    EXPECT_EQ(archive->dist_squared(a,b), 9.0) << "Distance squared calculation is incorrect.";
}

TEST_F(CvtMapElitesArchiveTest, AddVectors) {
    std::vector<double> a{1,2,3}, b{3,2,1};
    EXPECT_EQ(archive->add(a,b), (std::vector<double>{4,4,4})) << "Vector addition is incorrect.";
}

TEST_F(CvtMapElitesArchiveTest, ScalarMult) {
    std::vector<double> a{2,4,6};
    EXPECT_EQ(archive->scalar_mult(a, 0.5), (std::vector<double>{1,2,3})) << "Scalar multiplication is incorrect.";
}

TEST_F(CvtMapElitesArchiveTest, AveragePoints) {
    std::vector<std::vector<double>> pts{{0,0,0}, {2,2,2}};
    EXPECT_EQ(archive->average(pts), (std::vector<double>{1,1,1})) << "Average calculation is incorrect.";
}

TEST_F(CvtMapElitesArchiveTest, AverageEmpty) {
    std::vector<std::vector<double>> pts;
    EXPECT_EQ(archive->average(pts), std::vector<double>(descriptors, 0.0)) << "Average calculation is incorrect.";
}
...
TEST_F(CvtMapElitesArchiveTest, NearestCentroid) {
    ...
    EXPECT_EQ(archive->nearest(p, cents), 1u) << "Nearest centroid calculation is incorrect.";
}

TEST_F(CvtMapElitesArchiveTest, NearestCentroidThrowsOnEmpty) {
    EXPECT_THROW(archive->nearest({1,1,1}, {}), std::runtime_error) << "Expected exception when finding nearest centroid in empty list.";
}


TEST_F(CvtMapElitesArchiveTest, CentroidCountMatchesArchiveSize) {
    EXPECT_EQ(archive->getCentroids().size(), centroids) << "Centroid count does not match archive size.";
    EXPECT_EQ(archive->size(), centroids) << "Archive size does not match centroid count.";
}
...
TEST_F(CvtMapElitesArchiveTest, SetAndGetFromDescriptors) {
    ...
    EXPECT_EQ(out.first, eval1) << "Failed to retrieve correct evaluation result.";
...
TEST_F(CvtMapElitesArchiveTest, OverwriteArchiveCell) {
    ...
    EXPECT_EQ(result.first, eval2) << "Failed to retrieve correct evaluation result.";
}
...
TEST_F(CvtMapElitesArchiveTest, SameDescriptorSameIndex) {
    ...
    EXPECT_EQ(
        archive->getIndexForDescriptor(d1),
...
TEST_F(CvtMapElitesArchiveTest, InitializeCvtReinitializesCentroids) {
    // Use the fixture's archive and rng (remember fixture's constructor already called initialize_cvt once)
    // Capture current centroids
    auto before = archive->getCentroids();

    // Call initialize_cvt again with the same RNG instance (rng state has advanced from constructor)
    archive->initialize_cvt(rng);

    auto after = archive->getCentroids();

    ASSERT_EQ(before.size(), after.size()) << "Centroid count changed after re-initialization.";
    ...
    // It is extremely likely centroids changed; assert that at least one centroid coordinate differs.
    EXPECT_TRUE(anyDifferent) << "initialize_cvt did not change any centroid (unexpected).";
}

TEST_F(CvtMapElitesArchiveTest, InitializeCvtPopulatesWithinBounds) {
    // Create an archive with 0 iterations to get a minimal initial population via initialize_cvt in constructor
    Mutator::RNG localRng(999);
    Selector::MapElites::CvtMapElitesArchive arch(localRng, centroids, descriptors, minV, maxV, 0 /*iter*/, dots, a1, b1, a2, b2);

    // Capture centroids after construction
    const auto& initCentroids = arch.getCentroids();
    ASSERT_EQ(initCentroids.size(), centroids) << "Centroid count mismatch after initialization.";
    ...
    // Now call initialize_cvt explicitly again and verify bounds are still respected
    arch.initialize_cvt(localRng);
    const auto& newCentroids = arch.getCentroids();
    ASSERT_EQ(newCentroids.size(), centroids) << "Centroid count mismatch after re-initialization.";
