...
TEST_F(MapElitesArchiveTest, ArchiveLimitsSize) {
    EXPECT_EQ(archive->getArchiveLimits().size(), bins) << "Archive limits size mismatch.";
}

TEST_F(MapElitesArchiveTest, IndexWithinBounds) {
    EXPECT_EQ(archive->getIndexArchive(0.1), 0u) << "Index calculation incorrect.";
    EXPECT_EQ(archive->getIndexArchive(9.9), bins - 1) << "Index calculation incorrect.";
}

TEST_F(MapElitesArchiveTest, IndexOutOfBoundsClamps) {
    EXPECT_EQ(archive->getIndexArchive(-5), 0u) << "Index calculation incorrect.";
    EXPECT_EQ(archive->getIndexArchive(100), bins - 1) << "Index calculation incorrect.";
}
...
TEST_F(MapElitesArchiveTest, SetAndGetByIndices) {
    ...
    EXPECT_EQ(result.first, dummyEval) << "Failed to retrieve correct evaluation result.";
    EXPECT_EQ(result.second, dummyVertex) << "Failed to retrieve correct vertex.";
}
...
TEST_F(MapElitesArchiveTest, ContainsRoot) {
    ...
    EXPECT_TRUE(archive->containsRoot(dummyVertex)) << "Archive should contain the added root.";
    EXPECT_FALSE(archive->containsRoot(dummyVertex2)) << "Archive should not contain this root.";
}
...
TEST_F(MapElitesArchiveTest, RemoveRootDirect) {
    ...
    EXPECT_FALSE(archive->containsRoot(dummyVertex)) << "Archive should not contain the removed root.";
}
...
TEST_F(MapElitesArchiveTest, RemoveRootOnlyIfNotComplete) {
    ...
    archive->removeRootFromArchiveIfNotComplete(dummyVertex, 5); // 0 < 5 so it should remove
    EXPECT_FALSE(archive->containsRoot(dummyVertex)) << "Archive should not contain the removed root.";
}
...
TEST_F(MapElitesArchiveTest, GetVerticesInArchive) {
    ...
    EXPECT_TRUE(verts.count(dummyVertex)) << "Archive should contain the first vertex.";
    EXPECT_TRUE(verts.count(dummyVertex2)) << "Archive should contain the second vertex.";
}
