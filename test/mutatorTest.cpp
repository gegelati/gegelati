...
TEST_F(MutatorTest, RNG) {
	...
	// on its first use
	ASSERT_EQ(rng.getUnsignedInt64(0, 100), 24) << "Returned pseudo-random value changed with a known seed.";

	ASSERT_EQ(rng.getDouble(0, 1.0), 0.99214520962982877) << "Returned pseudo-random value changed with a known seed.";
}
...
TEST_F(MutatorTest, LineMutatorInitRandomCorrectLine1) {
	...
	ASSERT_NO_THROW(Mutator::LineMutator::initRandomCorrectLine(l0, rng)) << "Pseudo-Random correct line initialization failed within an environment where failure should not be possible.";
	...
	ASSERT_NO_THROW(Mutator::LineMutator::initRandomCorrectLine(l1, rng)) << "Pseudo-Random correct line initialization failed within an environment where failure should not be possible.";
	...
	ASSERT_NO_THROW(Mutator::LineMutator::initRandomCorrectLine(l2, rng)) << "Pseudo-Random correct line initialization failed within an environment where failure should not be possible.";
	ASSERT_NO_THROW(Mutator::LineMutator::initRandomCorrectLine(l3, rng)) << "Pseudo-Random correct line initialization failed within an environment where failure should not be possible.";

	// Add another pseudo-random lines to the program
	Program::Line& l4 = p->addNewLine();
	// Additionally covers additional uneeded operand (register) 
	ASSERT_NO_THROW(Mutator::LineMutator::initRandomCorrectLine(l4, rng)) << "Pseudo-Random correct line initialization failed within an environment where failure should not be possible.";
...
TEST_F(MutatorTest, ProgramMutatorInitProgram) {
	...
	ASSERT_NO_THROW(Mutator::ProgramMutator::initRandomProgram(*p, params, rng)) << "Empty Program Random init failed";
	ASSERT_EQ(p->getNbLines(), 31) << "Random number of line is not as expected (with known seed).";

	ASSERT_NO_THROW(Mutator::ProgramMutator::initRandomProgram(*p, params, rng)) << "Non-Empty Program Random init failed";
...
TEST_F(MutatorTest, ProgramMutatorMutateBehavior) {
	...
	rng.setSeed(0);
	ASSERT_TRUE(Mutator::ProgramMutator::mutateProgram(*p, params, rng)) << "Mutation did not occur with known seed.";
...
TEST_F(MutatorTest, TPGMutatorInitRandomTPG) {
	...
	ASSERT_NO_THROW(Mutator::TPGMutator::initRandomTPG(tpg, params, rng)) << "TPG Initialization failed.";
	...
	ASSERT_THROW(Mutator::TPGMutator::initRandomTPG(tpg, params, rng), std::runtime_error) << "TPG Initialization should fail with bad parameters.";
...
TEST_F(MutatorTest, TPGMutatorRemoveRandomEdge) {
	...
	Mutator::RNG rng;
	rng.setSeed(0);
	ASSERT_NO_THROW(Mutator::TPGMutator::removeRandomEdge(tpg, vertex0, rng)) << "Removing a random edge failed unexpectedly.";
...
TEST_F(MutatorTest, TPGMutatorAddRandomEdge) {
	...
	rng.setSeed(0);
	// Run the add
	ASSERT_NO_THROW(Mutator::TPGMutator::addRandomEdge(tpg, vertex2, edges, rng)) << "Adding an edge to the TPG should succeed.";
	...
	ASSERT_THROW(Mutator::TPGMutator::addRandomEdge(tpg, vertex2, { &newEdge }, rng), std::runtime_error) << "Picking an edge not belonging to the graph should fail.";
}
...
TEST_F(MutatorTest, TPGMutatorMutateEdgeDestination) {
	...
	Mutator::RNG rng;
	rng.setSeed(2);
	ASSERT_NO_THROW(Mutator::TPGMutator::mutateEdgeDestination(tpg, vertex0, &edge1, { &vertex3, &vertex4 }, { &vertex1, &vertex2 }, params, rng));
	// Check properties of the tpg
	...
	ASSERT_NO_THROW(Mutator::TPGMutator::mutateEdgeDestination(tpg, vertex0, &edge0, { &vertex3, &vertex4 }, { &vertex2 }, params, rng));
...
TEST_F(MutatorTest, TPGMutatorMutateOutgoingEdge) {
	...
	ASSERT_NO_THROW(Mutator::TPGMutator::mutateOutgoingEdge(tpg, arch, vertex0, &edge0, { &vertex0 }, { &vertex1 }, newPrograms, params, rng));
...
TEST_F(MutatorTest, TPGMutatorMutateTeam) {
	...
	ASSERT_NO_THROW(Mutator::TPGMutator::mutateTPGTeam(tpg, arch, vertex0, { &vertex0, &vertex4 }, { &vertex1, &vertex2, &vertex3 }, { &edge2 }, newPrograms, params, rng)) << "Mutate team should not fail in these conditions.";
...
TEST_F(MutatorTest, TPGMutatorMutateProgramBehaviorAgainstArchive) {
	...
	Mutator::TPGMutator::mutateOutgoingEdge(tpg, arch, vertex0, &edge0, { &vertex0 }, { &vertex1 }, newPrograms, params, rng);

	ASSERT_NO_THROW(Mutator::TPGMutator::mutateProgramBehaviorAgainstArchive(newPrograms.front(), params, arch, rng)) << "Mutating a Program behavior failed unexpectedly.";
