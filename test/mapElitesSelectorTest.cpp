...
{
    EXPECT_THROW(selector->getMapElitesArchiveAt(descriptor),
                 std::runtime_error);
}
...
{
    FakeMultiContinuousLearningEnvironment le;
    auto d2 = std::make_shared<
        Selector::MapElites::DefaultDescriptors::ActionValues>(); // NOT
                                                                  // initialized
...
TEST_F(MapElitesSelectorTest, CreateSelectionMetricsReturnsValidObject)
{
    ...
    auto casted = std::dynamic_pointer_cast<
        Selector::MapElites::MapElitesSelectionMetrics>(metrics);
...
TEST_F(MapElitesSelectorTest, DoSelectionCrashWrongMetrics)
{
    ...
    const EvoGraph::Vertex* root = &graph->addNewTeam();
...
TEST_F(MapElitesSelectorTest, DoSelectionInserts)
{
    ...
    // Use the constructor taking scores to set the selection score
    // deterministically.
    auto metrics_high =
        std::make_shared<Selector::MapElites::MapElitesSelectionMetrics>(100.0);
    auto metrics_low =
        std::make_shared<Selector::MapElites::MapElitesSelectionMetrics>(1.0);

    // Prepare two descriptor vectors that will map to two *different* archive
    // cells. Use descriptor->getNbDescriptors() to get right size.
    size_t nbDesc = descriptor->getNbDescriptors();
    ASSERT_GT(nbDesc, 0u);

    std::vector<double> desc_vec_low(nbDesc, 0.1); // will fall in low bins
    std::vector<double> desc_vec_high(
        nbDesc, 0.9); // will fall in high bins (different cell)

    // Inject mapDescriptors entries into metrics (test-only: use const_cast to
    // mutate). This ensures getMapDescriptors().at(descriptor) will return the
    // vector we want.
    {
        auto& map_low = const_cast<std::map<
            std::shared_ptr<const Selector::MapElites::MapElitesDescriptor>,
            std::vector<double>>&>(metrics_low->getMapDescriptors());
        map_low[descriptor] = desc_vec_low;
    }
    {
        auto& map_high = const_cast<std::map<
            std::shared_ptr<const Selector::MapElites::MapElitesDescriptor>,
            std::vector<double>>&>(metrics_high->getMapDescriptors());
    ...
    auto eval_high =
        std::make_shared<Learn::EvaluationResult>(metrics_high, 1u);
    ...
    const EvoGraph::Vertex* root_high = &graph->addNewTeam();
    const EvoGraph::Vertex* root_low = &graph->addNewTeam();
    ...
    // Run the selection — should insert both results into two distinct archive
    // cells.
    EXPECT_NO_THROW(selector->doSelection(results, rng));

    // After selection, check archive stored the correct evaluation+root for
...
TEST_F(MapElitesSelectorTest, DoSelectionReplaces)
{
    ...
    auto metrics_existing =
        std::make_shared<Selector::MapElites::MapElitesSelectionMetrics>(1.0);
    {
        auto& map_existing = const_cast<std::map<
            std::shared_ptr<const Selector::MapElites::MapElitesDescriptor>,
            std::vector<double>>&>(metrics_existing->getMapDescriptors());
        map_existing[descriptor] = desc_vec;
    }
    auto eval_existing =
        std::make_shared<Learn::EvaluationResult>(metrics_existing, 1u);
    const EvoGraph::Vertex* root_existing = &graph->addNewTeam();

    // Put the existing (low) value into the archive (simulating prior
    // generation). Note: archive is returned as shared_ptr<const
    // MapElitesArchive>, but our addArchiveFromDescriptor constructed a
    // non-const MapElitesArchive and getMapElitesArchiveAt returns
    // shared_ptr<const MapElitesArchive>. We need a non-const pointer to set
    // the archive. We re-fetch the non-const archive from selector's internal
    // map (safe test-only access via getMapElitesArchives()).
    ...
    nonConstArchive->setArchiveFromDescriptors(root_existing, eval_existing,
                                               desc_vec);

    // Now build a new, higher-scoring evaluation that should replace the
    // existing one
    auto metrics_new =
        std::make_shared<Selector::MapElites::MapElitesSelectionMetrics>(200.0);
    {
        auto& map_new = const_cast<std::map<
            std::shared_ptr<const Selector::MapElites::MapElitesDescriptor>,
            std::vector<double>>&>(metrics_new->getMapDescriptors());
        map_new[descriptor] = desc_vec;
    }
    auto eval_new = std::make_shared<Learn::EvaluationResult>(metrics_new, 1u);
    const EvoGraph::Vertex* root_new = &graph->addNewTeam();
    ...
    results.insert(
        {eval_existing,
         root_existing}); // include the old eval as well (it will not survive)

    // Run selection: since both map to the same descriptor, the new
    // higher-score should replace the old one.
    EXPECT_NO_THROW(selector->doSelection(results, rng));
...
TEST_F(MapElitesSelectorTest, TwoArchives)
{
    ...
    std::map<std::shared_ptr<const Selector::MapElites::MapElitesDescriptor>,
             std::vector<double>>
        mapA = {{descriptor, cell1_A}, {descriptorB, cell1_B}};
    std::map<std::shared_ptr<const Selector::MapElites::MapElitesDescriptor>,
             std::vector<double>>
        mapB = {{descriptor, cell2_A}, {descriptorB, cell2_B}};
    std::map<std::shared_ptr<const Selector::MapElites::MapElitesDescriptor>,
             std::vector<double>>
        mapAB = {{descriptor, cell2_A}, {descriptorB, cell1_B}};
    std::map<std::shared_ptr<const Selector::MapElites::MapElitesDescriptor>,
             std::vector<double>>
        mapNone = {{descriptor, cellNoneA}, {descriptorB, cellNoneB}};

    const EvoGraph::Vertex* vA = &graph->addNewAction(0);
    const EvoGraph::Vertex* vB = &graph->addNewAction(1);
    const EvoGraph::Vertex* vAB = &graph->addNewAction(2);
    const EvoGraph::Vertex* vNone = &graph->addNewAction(3);

    auto mA = std::make_shared<Selector::MapElites::MapElitesSelectionMetrics>(
        50.0, mapA);
    auto mB = std::make_shared<Selector::MapElites::MapElitesSelectionMetrics>(
        50.0, mapB);
    auto mAB = std::make_shared<Selector::MapElites::MapElitesSelectionMetrics>(
        100.0, mapAB);
    auto mNone =
        std::make_shared<Selector::MapElites::MapElitesSelectionMetrics>(
            5.0, mapNone);
