...
TEST_F(ExhaustiveArchiveTest, ConstructorDestructor) {
	Archive* a;
	ASSERT_NO_THROW(a = new ExhaustiveArchive();) << "Default construction of an ExhaustiveArchive failed";

	ASSERT_NO_THROW(delete a;) << "Destruction of an empty ExhaustiveArchive failed.";
}

TEST_F(ExhaustiveArchiveTest, AddRecordingTests) {
	ExhaustiveArchive archive;

	// Add a fictive recording
	ASSERT_NO_THROW(archive.addRecording(p, vect, 1.3)) << "Adding a recording to the empty ExhaustiveArchive failed.";

	ASSERT_EQ(archive.getNbRecordings(), 1) << "Number or recordings in the ExhaustiveArchive is incorrect.";
	ASSERT_EQ(archive.getNbDataHandlers(), 1) << "Number or dataHandlers copied in the archive is incorrect.";

	// Add other recordings with the same DataHandlers
	Program::Program p2(*e);
	ASSERT_NO_THROW(archive.addRecording(&p2, vect, 0.3)) << "Adding a recording to the non-empty ExhaustiveArchive failed.";
	ASSERT_EQ(archive.getNbRecordings(), 2) << "Number or recordings in the ExhaustiveArchive is incorrect.";
	ASSERT_EQ(archive.getNbDataHandlers(), 1) << "Number or dataHandlers copied in the ExhaustiveArchive is incorrect.";

	// Add another recording with a new environment
	// change data in one dataHandler
	DataHandlers::PrimitiveTypeArray<int>& d = (DataHandlers::PrimitiveTypeArray<int>&)vect.at(1).get();
	d.setDataAt(typeid(PrimitiveType<int>), 2, PrimitiveType<int>(1337));
	ASSERT_NO_THROW(archive.addRecording(p, vect, 0.2)) << "Adding a recording to the non-empty ExhaustiveArchive failed.";
	ASSERT_EQ(archive.getNbRecordings(), 3) << "Number or recordings in the ExhaustiveArchive is incorrect.";
	ASSERT_EQ(archive.getNbDataHandlers(), 2) << "Number or dataHandlers copied in the ExhaustiveArchive is incorrect.";
}

TEST_F(ExhaustiveArchiveTest, IsUnique) {
	ExhaustiveArchive archive;

	// Add a few fictive recordings (althoug it doesnt matter)
	archive.addRecording(p, vect, 1.0);
	archive.addRecording(p, vect, 1.5);
	DataHandlers::PrimitiveTypeArray<int>& d = (DataHandlers::PrimitiveTypeArray<int>&)vect.at(1).get();
	d.setDataAt(typeid(PrimitiveType<int>), 2, PrimitiveType<int>(1337));
	...
	ASSERT_THROW(archive.isRecordingExisting(archive.getCombinedHash(vect), p), std::runtime_error) << "Unicity test should not be available in ExhaustiveArchive.";
}

TEST_F(ExhaustiveArchiveTest, areProgramResultsUnique) {
	ExhaustiveArchive archive;

	// Add content (although it does'n matter)
	size_t hash1 = archive.getCombinedHash(vect);
	DataHandlers::PrimitiveTypeArray<int>& d = (DataHandlers::PrimitiveTypeArray<int>&)vect.at(1).get();
	...
	ASSERT_THROW(archive.areProgramResultsUnique({ {hash1, 3.0}, {hash2, 3.5} }), std::runtime_error) << "Unicity of program behavior should not be available in ExhaustiveArchive.";
}

TEST_F(ExhaustiveArchiveTest, DataHandlersAccessors) {
	ExhaustiveArchive archive;
...
TEST_F(ExhaustiveArchiveTest, Clear) {
	ExhaustiveArchive archive;
